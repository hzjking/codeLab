...
<head>
  ...
  <script type="text/javascript" language="javascript">
    ...
    function submit(){
      document.queryApplyDataParam.action = '<%=request.getContextPath()%>/apply/apply_data_param';
    ...
    function loockDetail(param){
      location.href="<%=request.getContextPath()%>/apply/apply_data_detail?id="+param;
    }
    function setvalue() {
      document.getElementById("type").value=document.getElementById("selectValue").value;
    }
...
<section>
  <div class="panel panel-default">
    ...
    <form id="queryApplyDataParam" name="queryApplyDataParam" action="<%=request.getContextPath()%>/apply/apply_data_param" method="post"  onsubmit="return checkAll()" >
      <div class="form-group">
        <div class="col-sm-3">
          <label class="control-label">申请用户:</label>
          <input id="userPhone" name="userPhone" value="${applyDataQueryDTO.phone}" type="text" placeholder="请输入手机号" class="form-control" onfocus="userPhoneFocus()"  onblur="userPhoneBlur()"/>
        </div>

        <div class="col-sm-3">
          <label class="control-label">申请人姓名:</label>
          <input id="userName" name="userName"  value="${applyDataQueryDTO.realName}" type="text" placeholder="请输入姓名" class="form-control" onfocus="userNameFocus()" onblur="userNameBlur()"/>
        </div>

        <div class="col-sm-3">
          <label class="control-label">经营名称:</label>
          <input id="shopName" name="shopName"  value="${applyDataQueryDTO.shopName}" type="text" placeholder="请输入经营名称" class="form-control" onfocus="userShopNameFocus()" onblur="userShopNameBlur()"/>
        </div>
      ...
      <div class="form-group">
        ...
        <div class="col-sm-3">
          ...
          <input id="applyId" name="applyId"  value="${applyDataQueryDTO.id}" type="text" placeholder="请输入申请编号" class="form-control" onfocus="userAppldIdFocus()" onblur="userAppldIdBlur()"/>
        </div>

        <div class="col-sm-3">
          <label class="control-label">期望融资金额：</label>
          <div class="input-group">
            <input type="text" placeholder="请输入起始融资金额"  id="expectedFinancingAmtStart" name="expectedFinancingAmtStart"  value="${applyDataQueryDTO.expectedFinancingAmtStart}" class="form-control"  onfocus="userAMTStartFocus()" onblur="userAMTStartBlur()">
            <input type="text" placeholder="请输入终止融资金额" class="form-control"  id="expectedFinancingAmtEnd"   name="expectedFinancingAmtEnd"   value="${applyDataQueryDTO.expectedFinancingAmtEnd}"   onfocus="userAMTEndFocus()" onblur="userAMTEndBlur()">
          </div>
        </div>
        ...
        <div class="col-sm-3">
          ...
          <div class="input-group">
            <input type="text" placeholder="请输入开始日期" class="form-control" id="startTime" name="startTime"  value="${applyDataQueryDTO.createTimeStart}" onfocus="userTimeStartFocus()" onblur="userTimeStartBlur()">
            <span class="input-group-addon"><i class="glyphicon glyphicon-calendar"></i></span>
          </div>

          <div class="input-group">
            <input type="text" placeholder="请输入结束日期" class="form-control" id="endTime" name="endTime" value="${applyDataQueryDTO.createTimeEnd}" onfocus="userTimeEndFocus()" onblur="userTimeEndBlur()">
            <span class="input-group-addon"><i class="glyphicon glyphicon-calendar"></i></span>
          </div>
          </div>
      ...
      <div class="form-group">
        <div class="col-sm-6">
          ...
          <input type="hidden" value="${applyDataQueryDTO.sandState}" id="selectValue"/>
          <select id="type" name="type" class="form-contol mb15"  onselect="">
          ...
          <table class="table  table-striped table-hover mb30" id="userTable">
            ...
            <c:if test="${!empty applyDatas}">
              <c:forEach items="${applyDatas}" var="apply">
                <tr class="odd gradeX" onclick="loockDetail('${apply.id}');">
                  <td>${apply.phone}</td>
